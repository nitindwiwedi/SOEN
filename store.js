...
const Project = () => {
  ...
  let runProcess = null; // Store running process globally
...
async function runProject() {
  ...
  if (htmlFile) {
    ...
    console.log("📂 Server file written!");

    // 🔴 Kill the previous process if it exists
    if (runProcess) {
      console.log("⚠️ Stopping previous process...");
      try {
    ...
    runProcess = await webContainer.spawn("node", ["server.js"]);
    ...
    // 🚀 Start new process
    runProcess = await webContainer.spawn("npm", ["start"]);
  ...
  return (
    <main className="h-screen w-screen flex bg-gray-100 overflow-hidden relative">
      ...
      <section
        className={`left absolute right-0 flex flex-col h-[89%] w-96 bg-white shadow-lg transition-all duration-300 ease-in-out z-10 ${
      ...
      <section className="right flex-grow bg-gray-50 h-full flex shadow-inner">
        ...
        {iframeUrl && webContainer && (
          <div
            className={`flex flex-col h-full border-l border-gray-300 shadow-md transition-all duration-300 ${
              isFullScreen ? "fixed inset-0 z-50 bg-white" : "min-w-96"
            }`}
          >
